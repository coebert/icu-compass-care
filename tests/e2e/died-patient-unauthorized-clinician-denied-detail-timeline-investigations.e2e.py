...
DETAILS, TIMELINE, or INVESTIGATIONS of a specific DIED patient record.
...
authenticated account that has NOT been granted a clinical role. Died records
are retained (never hard-deleted), so they must be protected exactly like live
ones.

The died patient is seeded with data on every surface the request names:
  - DETAIL       : name + confidential current_management note
...
Signed in as the unauthorized clinician, the test asserts:
  1. ROUTE/UI      — /patients/<id> never renders the patient's name, "Died"
                     status, or any seeded marker (in-app "not found" or /auth).
...
Run:  python3 tests/e2e/died-patient-unauthorized-clinician-denied-detail-timeline-investigations.e2e.py
...
MARKER = f"E2E-DIED-DENY-{int(time.time())}"
...
ALL_MARKERS = [SECRET_NOTE, EVENT_NOTE, INV_NOTE, MICRO_NOTE, PATIENT_NAME]
...
def main():
    ...
    try:
        ...
        # Positive control: the died record genuinely exists.
        assert read_patient(patient_id), "seed failed: died patient not readable by service"
        ...
        with sync_playwright() as pw:
            ...
            # ---- 1. UI: the died record must not render for this clinician ----
            ...
            page.screenshot(path=str(SCREENSHOTS / "died_unauth_clinician_denied.png"))
        ...
        print(
            "PASS: unauthorized clinician could not view or edit the died patient's "
