...
Treatment Escalation Plan (TEP) — setting `tep_in_place` and `tep_details` — on
an admitted patient, and those values then appear in the patient handover view.
...
This test reproduces that exact data path and rendering:
  ...
  1. Seeds an `admitted` patient (no TEP yet) via the admin API.
  2. Signs in as a throwaway CLINICIAN user.
  3. Records tep_in_place + tep_details via the RLS-enforced Data API (as the
     clinical user would from the UI).
  4. Re-fetches the patient list as that user, filters to the active/handover
     set, renders the handover "flags" string, and asserts the escalation-plan
     (TEP) value with its details is present.
...
Run:  python3 tests/e2e/clinical-user-tep-appears-in-handover.e2e.py
...
def create_clinical_user():
    email = f"e2e-tep-{STAMP}@example.com"
    r = requests.post(
...
def create_patient():
    ...
    r = requests.post(
        ...
        json={
            ...
            # Deliberately no TEP yet — the clinician records it below.
            "tep_in_place": False,
        },
        timeout=30,
    )
...
def main():
    ...
    try:
        ...
        # ---- 1. Clinician records the Treatment Escalation Plan ----
        upd = requests.patch(
            f"{SUPABASE_URL}/rest/v1/patients?id=eq.{patient_id}",
        ...
        assert upd.status_code in (200, 204), (
            f"clinical TEP update must succeed, got {upd.status_code}: {upd.text}"
        )
        body = upd.json()
        assert body and body[0].get("tep_in_place") is True, (
            f"tep_in_place should persist as True, got {body}"
        )
        assert body and body[0].get("tep_details") == TEP_DETAILS, (
            f"tep_details should persist, got {body}"
        )
        ...
        # ---- 3. The handover flags column must show the saved TEP value ----
        ...
        print(
            "PASS: clinical user's Treatment Escalation Plan (tep_in_place + "
            "tep_details) appears in the patient handover view"
        )
        return 0
